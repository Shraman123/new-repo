...
## 2026-09-19 — Phase 5 voice plan (docs/VOICE_PLAN.md only, no implementation)

- Studied `github.com/Shraman123/AI-Call-Centre` (cloned read-only to `/home/user/shraman123/ai-call-centre`, not committed anywhere). It's a self-hosted Asterisk+Whisper(`base`)+Ollama+`pyttsx3` demo with no telephony provider, no missed-call/callback logic, and no Bengali support (`pyttsx3` shells out to `espeak-ng`, weak for Bengali) — reusable as an architectural reference (greet/listen/transcribe/LLM/speak loop, a call log), not as a stack to adopt as-is.
- Proposed telephony provider: **Exotel** — its AgentStream/Voicebot WebSocket lets us bring our own STT/LLM/TTS over a real Indian number (Exotel does no STT/TTS itself), rather than us running SIP infrastructure. Costs in VOICE_PLAN.md are order-of-magnitude from web research (cited there), explicitly flagged as needing a real quote, not a number to build a budget on.
- Proposed STT/TTS: **Sarvam AI** (Saaras v3 STT, Bulbul v3 TTS) for Bengali/Banglish support, with Bhasini (govt-backed, free for non-commercial use) as a cost-reduction/fallback option to evaluate later.
- Proposed follow-up channel: **WhatsApp before SMS** — SMS to Indian numbers needs DLT template registration (TRAI), WhatsApp Business API doesn't; Exotel can carry both so no extra vendor.
- This is a planning document only, per PROMPTS.md's explicit "stop and wait for my OK" before touching `../vaani`. Nothing was implemented; the two concrete future code changes it calls out (a `channel` field on `/api/agent`'s request schema, a shorter/no-bracket system prompt variant for voice) are documented, not built.
